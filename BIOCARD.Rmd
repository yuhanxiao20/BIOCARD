...
model_vars = c("Age_CHAMPS", "bl_sex", "bl_APOE4", "bl_BMI", "bl_low", "bl_high",
               "bl_all","time")
...
data_champs_model <- data_champs_clean %>%
  ...
  fill(bl_BMI, .direction = "down") %>%
  fill(bl_low, .direction = "down") %>%
  fill(bl_high, .direction = "down") %>%
  fill(bl_all, .direction = "down") 
...
model_GFAP_low <- lmerTest::lmer(GFAP_zscore ~ time * (Age_CHAMPS + bl_sex + bl_APOE4 + bl_BMI + bl_low) + (1 + time | SUBJECT_ID), data = data_champs_GFAP)
model_NFL_low <- lmerTest::lmer(NFL_zscore ~ time * (Age_CHAMPS + bl_sex + bl_APOE4 + bl_BMI + bl_low) + (1 + time | SUBJECT_ID), data = data_champs_NFL)
model_PTAU181_low <- lmerTest::lmer(PTAU181_zscore ~ time * (Age_CHAMPS + bl_sex + bl_APOE4 + bl_BMI + bl_low) + (1 + time | SUBJECT_ID), data = data_champs_PTAU181)
model_AB42AB40_low <- lmerTest::lmer(AB42AB40_zscore ~ time * (Age_CHAMPS + bl_sex + bl_APOE4 + bl_BMI + bl_low) + (1 + time | SUBJECT_ID), data = data_champs_AB42AB40)
model_sTREM2_low <- lmerTest::lmer(sTREM2_zscore ~ time * (Age_CHAMPS + bl_sex + bl_APOE4 + bl_BMI + bl_low) + (1 + time | SUBJECT_ID), data = data_champs_sTREM2)
model_YKL40_low <- lmerTest::lmer(YKL40_zscore ~ time * (Age_CHAMPS + bl_sex + bl_APOE4 + bl_BMI + bl_low) + (1 + time | SUBJECT_ID), data = data_champs_YKL40)
...
In sTREM2 analysis, low intensity CHAMPS value is a predictor with a significance of 0.1.
```{r}
median_int <- median(data_champs_sTREM2$bl_low, na.rm = TRUE)

data_champs_sTREM2$int_group <- ifelse(data_champs_sTREM2$bl_low > median_int, "Above Median", "Below Median")

plot_below <- ggplot(data_champs_sTREM2[data_champs_sTREM2$int_group == "Below Median",], aes(x = time, y = sTREM2_zscore)) +
  geom_point(color = "blue") +
  geom_smooth(color = "blue") +
  labs(title = "Standardized Biomarker sTREM2 Values over Time \nSince Baseline (Below Median Low Intensity Value)",
       x = "Time (years)",
       y = "sTREM2 zscore") +
  theme_minimal()

plot_above <- ggplot(data_champs_sTREM2[data_champs_sTREM2$int_group == "Above Median",], aes(x = time, y = sTREM2_zscore)) +
...
In YKL40 analysis, low intensity CHAMPS value is a predictor with a significance of 0.1.
```{r}
median_int <- median(data_champs_YKL40$bl_low, na.rm = TRUE)

data_champs_YKL40$int_group <- ifelse(data_champs_YKL40$bl_low > median_int, "Above Median", "Below Median")

plot_below <- ggplot(data_champs_YKL40[data_champs_YKL40$int_group == "Below Median",], aes(x = time, y = YKL40_zscore)) +
  geom_point(color = "blue") +
  geom_smooth(color = "blue") +
  labs(title = "Standardized Biomarker YKL40 Values over Time \nSince Baseline (Below Median Low Intensity Value)",
       x = "Time (years)",
       y = "YKL40 zscore") +
  theme_minimal()

plot_above <- ggplot(data_champs_YKL40[data_champs_YKL40$int_group == "Above Median",], aes(x = time, y = YKL40_zscore)) +
  geom_point(color = "red") +
  geom_smooth(color = "red") +
  labs(title = "Standardized Biomarker YKL40 Values over Time \nSince Baseline (Above Median Low Intensity Value)",
       x = "Time (years)",
...
# High intensity
# Fit the linear mixed model
model_GFAP_high <- lmerTest::lmer(GFAP_zscore ~ time * (Age_CHAMPS + bl_sex + bl_APOE4 + bl_BMI + bl_high) + (1 + time | SUBJECT_ID), data = data_champs_GFAP)
model_NFL_high <- lmerTest::lmer(NFL_zscore ~ time * (Age_CHAMPS + bl_sex + bl_APOE4 + bl_BMI + bl_high) + (1 + time | SUBJECT_ID), data = data_champs_NFL)
model_PTAU181_high <- lmerTest::lmer(PTAU181_zscore ~ time * (Age_CHAMPS + bl_sex + bl_APOE4 + bl_BMI + bl_high) + (1 + time | SUBJECT_ID), data = data_champs_PTAU181)
model_AB42AB40_high <- lmerTest::lmer(AB42AB40_zscore ~ time * (Age_CHAMPS + bl_sex + bl_APOE4 + bl_BMI + bl_high) + (1 + time | SUBJECT_ID), data = data_champs_AB42AB40)
model_sTREM2_high <- lmerTest::lmer(sTREM2_zscore ~ time * (Age_CHAMPS + bl_sex + bl_APOE4 + bl_BMI + bl_high) + (1 + time | SUBJECT_ID), data = data_champs_sTREM2)
model_YKL40_high <- lmerTest::lmer(YKL40_zscore ~ time * (Age_CHAMPS + bl_sex + bl_APOE4 + bl_BMI + bl_high) + (1 + time | SUBJECT_ID), data = data_champs_YKL40)
...
```{r include = F}
# In NFL analysis, high intensity CHAMPS value is a predictor with a significance of 0.1.
median_int <- median(data_champs_NFL$bl_high, na.rm = TRUE)

data_champs_NFL$int_group <- ifelse(data_champs_NFL$bl_high > median_int, "Above Median", "Below Median")
...
# All intensity
# Fit the linear mixed model
model_GFAP_all <- lmerTest::lmer(GFAP_zscore ~ time * (Age_CHAMPS + bl_sex + bl_APOE4 + bl_BMI + bl_all) + (1 + time | SUBJECT_ID), data = data_champs_GFAP)
model_NFL_all <- lmerTest::lmer(NFL_zscore ~ time * (Age_CHAMPS + bl_sex + bl_APOE4 + bl_BMI + bl_all) + (1 + time | SUBJECT_ID), data = data_champs_NFL)
model_PTAU181_all <- lmerTest::lmer(PTAU181_zscore ~ time * (Age_CHAMPS + bl_sex + bl_APOE4 + bl_BMI + bl_all) + (1 + time | SUBJECT_ID), data = data_champs_PTAU181)
model_AB42AB40_all <- lmerTest::lmer(AB42AB40_zscore ~ time * (Age_CHAMPS + bl_sex + bl_APOE4 + bl_BMI + bl_all) + (1 + time | SUBJECT_ID), data = data_champs_AB42AB40)
model_sTREM2_all <- lmerTest::lmer(sTREM2_zscore ~ time * (Age_CHAMPS + bl_sex + bl_APOE4 + bl_BMI + bl_all) + (1 + time | SUBJECT_ID), data = data_champs_sTREM2)
model_YKL40_all <- lmerTest::lmer(YKL40_zscore ~ time * (Age_CHAMPS + bl_sex + bl_APOE4 + bl_BMI + bl_all) + (1 + time | SUBJECT_ID), data = data_champs_YKL40)
...
In NFL analysis, all intensity CHAMPS value is a predictor with a significance of 0.1.
```{r}
median_int <- median(data_champs_NFL$bl_all, na.rm = TRUE)

data_champs_NFL$int_group <- ifelse(data_champs_NFL$bl_all > median_int, "Above Median", "Below Median")
...
```{r}
# prepare data for regression analysis
model_vars = c("Age_Act", "bl_sex", "bl_APOE4", "bl_BMI", "bl_LTAC10","time")
...
```{r}
# Fit the linear mixed model
model_GFAP_act <- lmerTest::lmer(GFAP_zscore ~ time * (Age_Act + bl_sex + bl_APOE4 + bl_LTAC10) + (1 + time | SUBJECT_ID), data = data_act_GFAP)
model_NFL_act <- lmerTest::lmer(NFL_zscore ~ time * (Age_Act + bl_sex + bl_APOE4 + bl_LTAC10) + (1 + time | SUBJECT_ID), data = data_act_NFL)
model_PTAU181_act <- lmerTest::lmer(PTAU181_zscore ~ time * (Age_Act + bl_sex + bl_APOE4 + bl_LTAC10) + (1 + time | SUBJECT_ID), data = data_act_PTAU181)
model_AB42AB40_act <- lmerTest::lmer(AB42AB40_zscore ~ time * (Age_Act + bl_sex + bl_APOE4 + bl_LTAC10) + (1 + time | SUBJECT_ID), data = data_act_AB42AB40)
model_sTREM2_act <- lmerTest::lmer(sTREM2_zscore ~ time * (Age_Act + bl_sex + bl_APOE4 + bl_LTAC10) + (1 + time | SUBJECT_ID), data = data_act_sTREM2)
model_YKL40_act <- lmerTest::lmer(YKL40_zscore ~ time * (Age_Act + bl_sex + bl_APOE4 + bl_LTAC10) + (1 + time | SUBJECT_ID), data = data_act_YKL40)
...
In GFAP analysis, LTAC10 is a predictor with a significance of 0.05.
```{r}
median_LTAC10 <- median(data_act_GFAP$bl_LTAC10, na.rm = TRUE)

data_act_GFAP$LTAC10_group <- ifelse(data_act_GFAP$bl_LTAC10 > median_LTAC10, "Above Median", "Below Median")
data_act_GFAP$GFAP_pred <- predict(model_GFAP_act, newdata = data_act_GFAP, re.form = ~(1 | SUBJECT_ID))

plot_below <- ggplot(data_act_GFAP[data_act_GFAP$LTAC10_group == "Below Median",], aes(x = time, y = GFAP_zscore)) +
  geom_point(color = "blue") +
  geom_smooth(color = "blue") +
  labs(title = "Standardized Biomarker GFAP Values over Time \nSince Baseline (Below Median GFAP)",
       x = "Time (years)",
       y = "GFAP zscore") +
  theme_minimal()

plot_above <- ggplot(data_act_GFAP[data_act_GFAP$LTAC10_group == "Above Median",], aes(x = time, y = GFAP_zscore)) +
  geom_point(color = "red") +
  geom_smooth(color = "red") +
  labs(title = "Standardized Biomarker GFAP Values over Time \nSince Baseline (Above Median GFAP)",
       x = "Time (years)",
       y = "GFAP zscore") +
  theme_minimal()
grid.arrange(plot_below, plot_above, ncol = 2)
